...
Nilai berikut adalah **starting point default** saat template baru pertama kali dibuat/diunggah — admin tetap bebas mengubah posisi, ukuran, font, warna, dan alignment tiap elemen lewat Editor Visual (5.7a) kapan pun, dan perubahan itu tersimpan sebagai konfigurasi aktif untuk generate berikutnya.
...
- Font "Luxurious Script" harus benar-benar termuat (loaded) sebelum proses render ke canvas dilakukan, supaya hasil generate sertifikat konsisten dengan preview — bukan fallback ke font default browser/sistem.
- Kotak teks Nama Peserta default dibuat cukup lebar (mengikuti lebar area yang wajar di desain sertifikat) supaya nama panjang tidak terpotong; admin tetap bisa resize manual kalau perlu.
- Posisi default ketiga elemen diatur agar tidak saling bertumpuk/bertabrakan di layout sertifikat standar UPTD.
